    ...
    def get_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'data': {
                ...
                'categories': self.reader.get_categories(),
...
def main():
    ...
    parser.add_argument('--path', '-p', default=None, help='仓库路径；缺省自动探测本机 Obsidian 仓库')
    ...
    # 路径缺省时自动探测本机 Obsidian 默认仓库，不再硬编码单机路径
    vault_path = args.path
    if not vault_path:
        vault = pick_default_vault()
        if not vault:
            print("未找到 Obsidian 仓库。请先打开 Obsidian 创建/注册仓库，或用 --path 显式指定。")
            sys.exit(1)
