...
# PARA 风格仓库：目录名 -> 分类 slug
PARA_FOLDER_MAP = {
    '0-收件箱': 'inbox', '1-项目': 'project', '2-领域': 'area',
    '3-资源库': 'resource', '4-存档': 'archive', '5-卡片盒': 'card',
    '6-日记': 'diary', '7-模板': 'template', '8-素材库': 'asset',
    '9-插件': 'plugin',
}

# PARA 风格仓库：分类 slug -> 展示名
PARA_CATEGORY_NAMES = {
...
# 日记识别关键词（中英文，出现在顶层目录名或标题中即判定为日记）
DIARY_FOLDER_KEYWORDS = ('日记', 'diary', 'journal', 'daily', 'dailynote')
...
class VaultReader:
    ...
    def __init__(self, vault_path: str):
        ...
        self._structure_profile: Optional[Dict] = None
        self._ignored_dirs = {
    ...
    def _init_ocr(self):
        ...
        try:
            ...
            # 新版 PaddleOCR 移除了 show_log 参数，旧版需要它——先试新版签名，失败再回退
            try:
    ...
    def scan(self, force: bool = False) -> Dict[str, NoteMetadata]:
        ...
        self._notes.clear()
        self.detect_structure()  # 先探测仓库结构，_classify_folder 依赖它
    ...
    def detect_structure(self) -> Dict:
        """探测仓库结构，返回结构档案（惰性计算、缓存）。

        返回：
          {
            'name': 'para' | 'generic',      # 结构类型
            'para_compatible': bool,          # 是否为 PARA 风格
            'top_folders': [...],             # 顶层目录名（已排除忽略项）
            'category_names': {...},          # 分类 slug -> 展示名
          }
        PARA 风格：顶层出现"数字-"目录或命中 PARA 中文名 → 用 PARA 分类。
        其他结构：顶层目录名即分类（slug 化），不要求任何固定命名。
        """
        if self._structure_profile:
            return self._structure_profile
        ...
        numbered = [d for d in top_dirs if re.match(r'^\d+\s*[-_]\s*\S', d)]
        para_style = bool(numbered) or any(d in PARA_FOLDER_MAP for d in top_dirs)

        if para_style:
            category_names = dict(PARA_CATEGORY_NAMES)
        else:
            # 通用结构：分类 = 顶层目录名（slug 化），根目录文件归 inbox
            category_names = {self._slugify(d): d for d in top_dirs}
            category_names['inbox'] = '根目录'

        self._structure_profile = {
            'name': 'para' if para_style else 'generic',
            'para_compatible': para_style,
            'top_folders': sorted(top_dirs),
            'category_names': category_names,
        }
        return self._structure_profile

    def get_structure(self) -> Dict:
        """获取结构档案（供 get_info 等暴露给上层）。"""
        return self.detect_structure()

    def get_categories(self) -> Dict[str, str]:
        """获取分类映射（slug -> 展示名），随结构自适应。"""
        return self.detect_structure()['category_names']

    @staticmethod
    def _slugify(name: str) -> str:
        """目录名 -> 分类 slug：去符号、小写（保留中英文）。"""
    ...
    def _is_diary(self, metadata: NoteMetadata) -> bool:
        ...
        top_folder = (metadata.folder.split('/')[0] if metadata.folder else '').lower()

        # 通用日记目录/标题关键词（中英文），顶层目录命中或标题命中即判定
        for kw in DIARY_FOLDER_KEYWORDS:
            if kw in top_folder or kw in title.lower():
                return True
        # 兼容旧版：完整路径里出现“日记”（如 4-存档/日记2024/…）
        if '日记' in folder:
            return True
    ...
    def _classify_folder(self, folder: str) -> str:
        """结构自适应分类：
        - PARA 风格：按 PARA_FOLDER_MAP 映射，未知目录归 'other'。
        - 通用结构：顶层目录名即分类（slug 化）；根目录文件归 'inbox'。
        """
        if not folder:
            return 'inbox'

        top_folder = folder.split('/')[0]
        profile = self.detect_structure()

        if profile['para_compatible']:
            return PARA_FOLDER_MAP.get(top_folder, 'other')

        return self._slugify(top_folder) or 'other'
